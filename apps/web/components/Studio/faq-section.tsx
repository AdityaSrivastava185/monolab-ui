    ...
    <div className="w-full flex justify-center items-start" style={{ 
      ...
      <div className="flex-1 px-4 md:px-12 py-16 md:py-20 flex flex-col lg:flex-row justify-start items-start gap-6 lg:gap-12">
        ...
        <div className="w-full lg:flex-1 flex flex-col justify-center items-start gap-4 lg:py-5">
          <div className="w-full flex flex-col justify-center font-semibold leading-tight md:leading-[44px] font-sans text-4xl tracking-tight" style={{ color: "var(--section-text, #49423D)" }}>
            Frequently Asked Questions
          </div>
          <div className="w-full text-base font-normal leading-7 font-sans" style={{ color: "var(--section-text, #605A57)" }}>
        ...
        <div className="w-full lg:flex-1 flex flex-col justify-center items-center">
          <div className="w-full flex flex-col">
            {faqData.map((item, index) => {
              ...
              return (
                <div key={index} className="w-full overflow-hidden" style={{ borderBottom: "1px solid var(--section-border, rgba(73,66,61,0.16))" }}>
                  <button
                    onClick={() => toggleItem(index)}
                    className="w-full px-5 py-[18px] flex justify-between items-center gap-5 text-left transition-colors duration-200"
                    style={{ backgroundColor: "transparent", color: "var(--section-text, #49423D)" }}
                    aria-expanded={isOpen}
                  >
                    <div className="flex-1 text-base font-medium leading-6 font-sans" style={{ color: "var(--section-text, #49423D)" }}>
                      {item.question}
                    </div>
                  ...
                  >
                    <div className="px-5 pb-[18px] text-sm font-normal leading-6 font-sans" style={{ color: "var(--section-text, #605A57)" }}>
                      {item.answer}
                    </div>
                  </div>
                </div>
